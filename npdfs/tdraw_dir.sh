...
function write_draw_file()
{
	...
	echo "#xrange 1, 20" >> ${outputfile}
	echo "#line 1, 1, 20, 1, 1, 9, 1" >> ${outputfile}
	echo "${inputdir}/pPb_npdf_compare_uncerts_ratio_tdraw.root         :epps16_uncerts_ratio_noErr :p l +w3 +k1 : title=#pi^{0} #hat{p_{T}} > 2 " >> ${outputfile}
	echo "${inputdir}/pPb_npdf_compare_uncerts_ratio_tdraw.root         :epps16_uncerts_ratio :p serror noleg -k: title=Graph" >> ${outputfile}
	echo "${inputdir}/pPb_npdf_compare_charm_uncerts_ratio_tdraw.root   :epps16_uncerts_ratio_noErr :p l +w3 +k2 : title=D^{0} #hat{p_{T}} > 2 " >> ${outputfile}
	echo "${inputdir}/pPb_npdf_compare_charm_uncerts_ratio_tdraw.root   :epps16_uncerts_ratio :p serror noleg -k: title=Graph" >> ${outputfile}
	echo "${inputdir}/pPb_npdf_compare_photon_uncerts_ratio_tdraw.root  :epps16_uncerts_ratio_noErr :p l +w3 +k4 : title=#gamma #hat{p_{T}} > 2 " >> ${outputfile}
	echo "${inputdir}/pPb_npdf_compare_photon_uncerts_ratio_tdraw.root  :epps16_uncerts_ratio :p serror noleg -k: title=Graph" >> ${outputfile}
}
...
write_draw_file ${indir} ${draw_file}
