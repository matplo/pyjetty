...
SUFFIX=$(echo $INPUT_FILE | cut -d/ -f5-11)
echo $SUFFIX
...
CONFIG_FILE=/software/users/james/pyjetty/pyjetty/alice_analysis/config/ml/ppAA_alice.yaml
cd /software/users/james/pyjetty/pyjetty/alice_analysis
pipenv run python process/user/ml/process_ppAA.py -f $INPUT_FILE -o $OUTPUT_DIR -c $CONFIG_FILE
