...
class roounfold_ang(analysis_base.analysis_base):
  ...
  def __init__(self, input_file_data='', input_file_response='', config_file='',
    ...
    super(roounfold_ang, self).__init__(input_file_data, input_file_response, config_file,
  ...
  def roounfold_ang(self):
    ...
    for jetR in self.jetR_list:
      for beta in self.beta_list:
        ...
        self.unfoldSingleOutputList(jetR, beta, label)
  ...
  def initialize_config(self):
    ...
    with open(self.config_file, 'r') as stream:
      ...
      # Retrieve list of configuration settings
  ...
  def get_responses(self, rebin_response=False):
    ...
    for jetR in self.jetR_list:
      for beta in self.beta_list:
        ...
        n_bins_det = getattr(self, 'n_bins_det_%s' % label)
  ...
  ###################################################################################################
  def unfoldSingleOutputList(self, jetR, beta, label):

    print('jetR = %s, beta = %s' % (jetR, beta))
    ...
    self.plot_kinematic_efficiency(jetR, beta, label)
    ...
    name = 'hMC_Det_%s' % label
    hMC_Det = self.get_MCdet2D(jetR, beta, label)
    setattr(self, name, hMC_Det)

    name = 'hMC_Truth_%s' % label
    hMC_Truth = self.get_MCtruth2D(jetR, beta, label)
    setattr(self, name, hMC_Truth)

    # Compute tagging rate
    self.compute_tagging_rate(jetR, beta, label)

    # Unfold spectrum
    if hData_PerBin and hMC_Det and hMC_Truth:
      self.unfoldJetSpectrum(jetR, beta, label)

  #################################################################################################
  # Compute tagging rate, based on MC correction
  #################################################################################################
  def compute_tagging_rate(self, jetR, beta, label):
    ...
    name = 'hTaggingFractions_%s' % label
    ...
    for bin in range(1, n_pt_bins_truth - 3):
      ...
      fraction_tagged = self.get_tagging_rate(jetR, min_pt_truth, max_pt_truth,
    ...
    fResult_name = getattr(self, 'fResult_name_%s' % label)
  ...
  # Unfold jet spectrum
  #################################################################################################
  def unfoldJetSpectrum(self, jetR, beta, label):

    self.utils.set_plotting_options()
    ROOT.gROOT.ForceStyle()

    # Create canvas to superpose iterations on one plot, to examine convergence
    c1 = ROOT.TCanvas('c1_%s' % label, 'c1_%s: histos' % label, 600, 450)
    c1.cd()
    c1.SetLogy()
    ...
    response = getattr(self, 'roounfold_response_%s' % label)

    name_data = getattr(self, 'name_data_rebinned_%s' % label)
    ...
    for i in range(1, reg_param_final + 3):
      ...
      name = 'hUnfolded_{}_{}_{}'.format(self.observable, label, i)
      hUnfolded.SetName(name)
      setattr(self, name, hUnfolded)
      hUnfolded.SetDirectory(0)

      # Correct by kinematic efficiency
      hKinematicEfficiency = getattr(self, 'hKinematicEfficiency_%s' % label)
    ...
    self.plot_unfolded_observable(jetR, beta, label)

    self.plot_unfolded_pt(jetR, beta, label)
    ...
    name_data = getattr(self, 'name_data_rebinned_%s' % label)
    hData = getattr(self, name_data)
    hMC_Det = getattr(self, 'hMC_Det_%s' % label)
    ...
    for i in range(1, reg_param_final + 3):
      ...
      # Apply RM to unfolded result, and check that I obtain measured spectrum (simple technical check)
                  self.plot_refolding_test(i, jetR, beta, label)

      # Unfold the smeared det-level result with response, and compare to truth-level MC.
                  self.plot_closure_test(i, jetR, beta, label)

  #################################################################################################
  # Plot various slices of the response matrix (from the THn)
  #################################################################################################
  def plot_unfolded_observable(self, jetR, beta, label):
    ...
    for i in range(1, n_pt_bins_truth-3):
      ...
      self.plot_observable(jetR, beta, label, min_pt_truth, max_pt_truth)
      self.plot_observable(jetR, beta, label, min_pt_truth, max_pt_truth, option = 'ratio')
      self.plot_observable(jetR, beta, label, min_pt_truth, max_pt_truth, option = 'uncertainties')

  #################################################################################################
  # Plot various slices of the response matrix (from the THn)
  #################################################################################################
  def plot_observable(self, jetR, beta, label, min_pt_truth, max_pt_truth, option=''):

    self.utils.set_plotting_options()
    ROOT.gROOT.ForceStyle()

    name = 'cResult{}_{}_{}-{}'.format(option, label, min_pt_truth, max_pt_truth)
    ...
    reg_param_final = 5 #self.utils.get_reg_param(self.sd_settings, self.sd_config_list, self.sd_config_dict, sd_label, self.observable, jetR)

    for i in range(1, reg_param_final + 3):

      h = self.get_unfolded_result(label, i, min_pt_truth, max_pt_truth, option)

      if option == 'ratio':
        if i > 1:
          h_previous = self.get_unfolded_result(label, i-1, min_pt_truth, max_pt_truth, option)
          h.Divide(h_previous)
        else:
          continue

      if option == 'uncertainties':
        h = self.get_unfolded_result_uncertainties(label, i, min_pt_truth, max_pt_truth, option)
    ...
    text_latex = ROOT.TLatex()
    text_latex.SetNDC()
    text = 'R = ' + str(jetR) + '   z_{cut} = ' + str(config["zcut"]) + '   sd_#beta = ' + str(config["sd_beta"])
    text_latex.DrawLatex(0.25, 0.75, text)

    outputFilename = os.path.join(self.output_dir,
                                  'hUnfolded{}_{}_{}_{}-{}{}'.format(option, self.observable, label,
  ...
  #################################################################################################
  def get_unfolded_result(self, label, i, min_pt_truth, max_pt_truth, option):
  ...
  #################################################################################################
  def get_unfolded_result_uncertainties(self, label, i, min_pt_truth, max_pt_truth, option):
  ...
  # Plot various slices of the response matrix (from the THn)
  #################################################################################################
  def plot_unfolded_pt(self, jetR, beta, label):
    ...
    reg_param_final = self.utils.get_reg_param(self.sd_settings, self.sd_config_list,
                                               self.sd_config_dict, sd_label, self.observable, jetR)
    ...
    text_latex = ROOT.TLatex()
    text_latex.SetNDC()
    text = "R = " + str(jetR) + "   z_{cut} = " + str(config["zcut"]) + "   sd_#beta = " + str(config["beta"])
    text_latex.DrawLatex(0.25, 0.75, text)

    outputFilename = os.path.join(self.output_dir, "hUnfoldedPt_{}_{}{}".format(self.observable, label,
                                                                                self.file_format))
    c.SaveAs(outputFilename)
    c.Close()
  ...
  def plot_kinematic_efficiency(self, jetR, beta, label):
    ...
    hResponse.SetName('hResponse_KinEff_JetPt_Obs_%s' % label)
    
    # Denominator -- by default, under/over-flow bins are included in projection
    hDenominator = hResponse.Projection(3, 1)
    hDenominator.SetName('%s_Denominator' % hDenominator.GetName())
    ...
    hKinematicEfficiency.SetName('hKinematicEfficiency_%s' % label)
    ...
    outputFilename = os.path.join(self.output_dir,
                                  'hKinematicEfficiency2D_%s%s' % (label, self.file_format))
    self.utils.plot_hist(hKinematicEfficiency, outputFilename, 'colz')
    
    # Save kinematic efficiency as class member
    setattr(self, 'hKinematicEfficiency_%s' % label, hKinematicEfficiency)
    
    # Plot 1D kinematic efficiency
    self.plot_kinematic_efficiency_projections(hKinematicEfficiency, jetR, beta, label)

  #################################################################################################
  # Unfold jet spectrum
  #################################################################################################
  def plot_kinematic_efficiency_projections(self, hKinematicEfficiency2D, jetR, beta, label):
    
    self.utils.set_plotting_options()
    ROOT.gROOT.ForceStyle()
    
    name = 'cKinEff_%s' % label
    c = ROOT.TCanvas(name, name, 600, 450)
    c.Draw()
    ...
    for bin in range(1, n_pt_bins_truth-3):
      ...
      name = 'hKinematicEfficiency_{}_{}-{}'.format(label, min_pt_truth, max_pt_truth)
      h.SetName(name)
    ...
    text_latex = ROOT.TLatex()
    text_latex.SetNDC()
    text = 'z_{cut} = ' + str(config["zcut"]) + '   sd_#beta = ' + str(config["sd_beta"])
  ...
  # Plot various slices of the response matrix (from the THn)
  #################################################################################################
  def plot_RM_slices(self, jetR, label):
    ...
    # (pt-det, pt-true, obs-det, obs-true)
    name_response = getattr(self, 'name_thn_%s' % label)
    ...
    for bin in range(1, n_pt_bins_truth-1):
      ...
      self.plot_Obs_Response(jetR, label, min_pt_truth, max_pt_truth, hResponse, output_dir_RM)

  #################################################################################################
  # Plot 2D SD-observable response for a fixed range of pt-truth
  #################################################################################################
  def plot_Obs_Response(self, jetR, label, min_pt_truth, max_pt_truth, hResponse, output_dir_RM):
    ...
    hResponse_Obs.SetName('hResponse_Obs_%s_%i_%i' % (label, int(min_pt_truth), int(max_pt_truth)))
    ...
    outputFilename = os.path.join(output_dir_RM, '%s%s' % (hResponse_Obs.GetName(), self.file_format))
  ...
  def normalizeResponseMatrix(self, hResponseMatrix):
    ...
    hTruthProjectionBefore = hResponseMatrix.ProjectionY('%s_py' % hResponseMatrix.GetName(), 1, 
                                                         hResponseMatrix.GetNbinsX()) # Do exclude under and overflow bins
  ...
  def plot_refolding_test(self, i, jetR, beta, label):
    ...
    hUnfolded = getattr(self, 'hUnfolded_{}_{}_{}'.format(self.observable, label, i))
    response = getattr(self, 'roounfold_response_%s' % label)
    ...
    for bin in range(2, n_pt_bins_det):
      ...
      self.plot_sd_refolded_slice(hFoldedTruth, i, jetR, beta, label, min_pt_det,
                                  max_pt_det, output_dir_refolding)

    self.plot_pt_refolded_slice(hFoldedTruth, i, jetR, beta, label, det_pt_bin_array[2],
                                det_pt_bin_array[-1], output_dir_refolding)

  #################################################################################################
  # Apply RM to unfolded result, and check that I obtain measured spectrum (simple technical check)
  #################################################################################################
  def plot_sd_refolded_slice(self, hFoldedTruth, i, jetR, beta, label, min_pt_det,
                             max_pt_det, output_dir_refolding):
    
    hFoldedTruth.GetXaxis().SetRangeUser(min_pt_det, max_pt_det)
    hFolded_obs = hFoldedTruth.ProjectionY()
    hFolded_obs.SetName('hFolded_obs_{}_{}_{}-{}'.format(label, i, min_pt_det, max_pt_det))
    ...
    hData_obs.SetName('hData_obs_{}_{}_{}-{}'.format(label, i, min_pt_det, max_pt_det))
    ...
    outputFilename = os.path.join(output_dir_refolding,
                                  'hFoldedTruth_{}_{}-{}_{}{}'.format(label, int(min_pt_det),
                                                                      int(max_pt_det), i, self.file_format))
    self.plot_sd_obs_ratio(hFolded_obs, hData_obs, None, self.ytitle, ratioYAxisTitle,
                           int(min_pt_det), int(max_pt_det), jetR, beta, label,
                           outputFilename, 'width', legendTitle, h1LegendLabel, h2LegendLabel)

  #################################################################################################
  # Apply RM to unfolded result, and check that I obtain measured spectrum (simple technical check)
  #################################################################################################
  def plot_pt_refolded_slice(self, hFoldedTruth, i, jetR, beta, label,
                             min_pt_det, max_pt_det, output_dir_refolding):
    
    hFoldedTruth.GetXaxis().SetRangeUser(min_pt_det, max_pt_det)
    hFolded_pt = hFoldedTruth.ProjectionX()
    hFolded_pt.SetName('hFolded_pt_{}_{}_{}-{}'.format(label, i, min_pt_det, max_pt_det))
    
    name_data_rebinned = getattr(self, 'name_data_rebinned_%s' % label)
    hData_PerBin = getattr(self, name_data_rebinned)
    hData_PerBin.GetXaxis().SetRangeUser(min_pt_det, max_pt_det)
    hData_pt = hData_PerBin.ProjectionX()
    hData_pt.SetName('hData_pt_{}_{}_{}-{}'.format(label, i, min_pt_det, max_pt_det))
    ...
    ratioYAxisTitle = 'Folded truth / Measured'
    outputFilename = os.path.join(output_dir_refolding,
                                  'hFoldedTruth_pt_{}_{}{}'.format(label, i, self.file_format))
    self.plot_sd_obs_ratio(hFolded_pt, hData_pt, None, self.ytitle, ratioYAxisTitle,
                           0, 0, jetR, beta, label, outputFilename, 'width',
                           legendTitle, h1LegendLabel, h2LegendLabel)

  #################################################################################################
  # Closure test
  #################################################################################################
  def plot_closure_test(self, i, jetR, beta, label):
    ...
    response = getattr(self, 'roounfold_response_%s' % label)
    hMC_Det = getattr(self, 'hMC_Det_%s' % label)
    hMC_Truth = getattr(self, 'hMC_Truth_%s' % label)
    ...
    self.plot_pt_closure_slice(hUnfolded2, hMC_Truth, i, jetR, beta, label, truth_pt_bin_array[1],
                               truth_pt_bin_array[-4], output_dir_closure)

  #################################################################################################
  # Apply RM to unfolded result, and check that I obtain measured spectrum (simple technical check)
  #################################################################################################
  def plot_sd_closure_slice(self, hUnfolded, hMC_Truth, i, jetR, beta, label,
                            min_pt_truth, max_pt_truth, output_dir_closure):
    
    hUnfolded.GetXaxis().SetRangeUser(min_pt_truth, max_pt_truth)
    hUnfolded_obs = hUnfolded.ProjectionY()
    hUnfolded_obs.SetName('hUnfolded_obs_{}_{}_{}-{}'.format(label, i, min_pt_truth, max_pt_truth))
    
    hMC_Truth.GetXaxis().SetRangeUser(min_pt_truth, max_pt_truth)
    hMCTruth_obs = hMC_Truth.ProjectionY()
    hMCTruth_obs.SetName('hMCTruth_obs_{}_{}_{}-{}'.format(label, i, min_pt_truth, max_pt_truth))
    ...
    outputFilename = os.path.join(output_dir_closure,
                                  'hClosure_{}_{}-{}_{}{}'.format(label, int(min_pt_truth),
                                                                  int(max_pt_truth), i, self.file_format))
    self.plot_sd_obs_ratio(hUnfolded_obs, hMCTruth_obs, None, self.ytitle, ratioYAxisTitle,
                           min_pt_truth, max_pt_truth, jetR, beta, label, outputFilename,
                           'width', legendTitle, h1LegendLabel, h2LegendLabel)

  #################################################################################################
  # Apply RM to unfolded result, and check that I obtain measured spectrum (simple technical check)
  #################################################################################################
  def plot_pt_closure_slice(self, hUnfolded, hMC_Truth, i, jetR, beta, label,
                            min_pt_truth, max_pt_truth, output_dir_closure):
  
    hUnfolded.GetXaxis().SetRangeUser(min_pt_truth, max_pt_truth)
    hUnfolded_pt = hUnfolded.ProjectionX()
    hUnfolded_pt.SetName('hUnfolded_pt_{}_{}'.format(label, i))
    
    hMC_Truth.GetXaxis().SetRangeUser(min_pt_truth, max_pt_truth)
    hMCTruth_pt = hMC_Truth.ProjectionX()
    hMCTruth_pt.SetName('hMCTruth_pt_{}_{}_'.format(label, i))
    ...
    ratioYAxisTitle = 'Unfolded MC det / Truth'
    outputFilename = os.path.join(output_dir_closure,
                                  'hClosure_pt_{}_{}{}'.format(label, i, self.file_format))
    self.plot_sd_obs_ratio(hUnfolded_pt, hMCTruth_pt, None, self.ytitle, ratioYAxisTitle, 0, 0, jetR, beta, 
                           label, outputFilename, 'width', legendTitle, h1LegendLabel, h2LegendLabel)
  ...
  #################################################################################################
  def get_MCdet2D(self, jetR, label):
    ...
    hResponse4D = hResponse.Clone()
    hResponse4D.SetName('%s_clone' % hResponse4D.GetName())
    
    hMC_Det = hResponse4D.Projection(2,0)
    hMC_Det.SetName('hMC_Det_%s' % label)
    return hMC_Det

  #################################################################################################
  # Get MC-det 2D projection
  #################################################################################################
  def get_MCtruth2D(self, jetR, label):
    ...
    hMC_Truth.SetName('hMC_Truth_%s' % label)
    return hMC_Truth

  #################################################################################################
  # Plot spectra and ratio of h (and h3, if supplied) to h2
  #################################################################################################
  def plot_sd_obs_ratio(self, h, h2, h3, yAxisTitle, ratioYAxisTitle, min_pt_det, max_pt_det,
                        jetR, beta, label, outputFilename, scalingOptions = "", legendTitle = "",
    ...
    text_latex = ROOT.TLatex()
    text_latex.SetNDC()
    text = 'z_{cut} = ' + str(label["zcut"]) + '   sd_#beta = ' + str(config["sd_beta"])
