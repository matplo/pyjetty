...
################################################################
class run_rg_analysis(common_base.common_base):
  ...
  def run_rg_analysis(self):
    ...
    for jetR in self.jetR_list:
      for sd_setting in self.sd_settings:
        ...
        if self.do_systematics:
          self.compute_systematics(jetR, sd_label)

        if self.do_plot_final_result:
          self.plot_final_result(jetR, sd_label, zcut, beta)
  ...
  def perform_unfolding(self):
    ...
    # Main result
    analysis_main = roounfold_rg.roounfold_rg(self.main_data, self.main_response, self.config_file, self.output_dir_main, self.file_format, rebin_response=self.check_rebin_response(self.output_dir_main))
    analysis_main.roounfold_rg()

    # Tracking efficiency variation
    if self.kTrackEff in self.systematics_list:
      analysis_trkeff = roounfold_rg.roounfold_rg(self.trkeff_data, self.trkeff_response, self.config_file, self.output_dir_trkeff, self.file_format, rebin_response=self.check_rebin_response(self.output_dir_trkeff))
      analysis_trkeff.roounfold_rg()

    # Prior variation
    if self.kPrior1 in self.systematics_list:
      analysis_prior = roounfold_rg.roounfold_rg(self.main_data, self.main_response, self.config_file, self.output_dir_prior, self.file_format, rebin_response=self.check_rebin_response(self.output_dir_prior), power_law_offset=0.5)
  ...
  #----------------------------------------------------------------------
  def compute_systematics(self, jetR, sd_label):
    print('Compute systematics...')

    # Get main result
    path_main = os.path.join(self.output_dir_main, 'fResult_R{}_{}.root'.format(jetR, sd_label))
    fMain = ROOT.TFile(path_main, 'READ')
    
    name = 'hUnfolded_R{}_{}_{}'.format(jetR, sd_label, self.reg_param_final)
    ...
    name = 'hUnfolded_R{}_{}_{}'.format(jetR, sd_label, self.reg_param_final+2)
    ...
    # Get trkeff result
    path_trkeff = os.path.join(self.output_dir_trkeff, 'fResult_R{}_{}.root'.format(jetR, sd_label))
    ...
    # Get prior result
    path_prior1 = os.path.join(self.output_dir_prior, 'fResult_R{}_{}.root'.format(jetR, sd_label))
    ...
    for bin in range(1, n_pt_bins_truth-3):
      ...
      self.compute_theta_systematic(jetR, sd_label, min_pt_truth, max_pt_truth)

  #----------------------------------------------------------------------
  def get_theta_distribution(self, jetR, sd_label, name2D, name1D, min_pt_truth, max_pt_truth):
  ...
  #----------------------------------------------------------------------
  def compute_theta_systematic(self, jetR, sd_label, min_pt_truth, max_pt_truth):
    ...
    # Get main histogram
    name2D = 'hUnfolded_R{}_{}_{}'.format(jetR, sd_label, self.reg_param_final)
    name1D = 'hMain_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    hMain = self.get_theta_distribution(jetR, sd_label, name2D, name1D, min_pt_truth, max_pt_truth)

    # Get reg param +2
    name2D = 'hUnfolded_R{}_{}_{}'.format(jetR, sd_label, self.reg_param_final+2)
    name1D = 'hRegParam1_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    hRegParam1 = self.get_theta_distribution(jetR, sd_label, name2D, name1D, min_pt_truth, max_pt_truth)
    
    # Get reg param -2
    name2D = 'hUnfolded_R{}_{}_{}'.format(jetR, sd_label, self.reg_param_final-2)
    name1D = 'hRegParam2_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    hRegParam2 = self.get_theta_distribution(jetR, sd_label, name2D, name1D, min_pt_truth, max_pt_truth)

    # Get trk eff
    name2D = 'hUnfolded_R{}_{}_{}_trkeff'.format(jetR, sd_label, self.reg_param_final)
    name1D = 'hTrkEff_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    hTrkEff = self.get_theta_distribution(jetR, sd_label, name2D, name1D, min_pt_truth, max_pt_truth)
    
    # Get prior1
    name2D = 'hUnfolded_R{}_{}_{}_prior1'.format(jetR, sd_label, self.reg_param_final)
    name1D = 'hPrior1_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    hPrior1 = self.get_theta_distribution(jetR, sd_label, name2D, name1D, min_pt_truth, max_pt_truth)
    
    #------------------------------------
    # Compute systematics

    # Reg param +2
    name = 'hSystematic_RegParam1_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    ...
    # Reg param -2
    name = 'hSystematic_RegParam2_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    ...
    name = 'hSystematic_RegParam_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    ...
    outputFilename = os.path.join(self.output_dir_systematics, 'hSystematic_RegParam_R{}_{}_{}-{}{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth, self.file_format))
    self.utils.plot_hist(hSystematic_RegParam, outputFilename, 'P E')
    
    # Prior
    name = 'hSystematic_Prior1_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    ...
    outputFilename = os.path.join(self.output_dir_systematics, 'hSystematic_Prior1_R{}_{}_{}-{}{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth, self.file_format))
    ...
    name = 'hResult_shape_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    ...
    # Trk eff
    name = 'hSystematic_TrkEff_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    ...
    outputFilename = os.path.join(self.output_dir_systematics, 'hSystematic_TrkEff_R{}_{}_{}-{}{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth, self.file_format))
    self.utils.plot_hist(hSystematic_TrkEff, outputFilename, 'P E')
  
    name = 'hResult_trkeff_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
  ...
  #----------------------------------------------------------------------
  def plot_final_result(self, jetR, sd_label, zcut, beta):
    ...
    for bin in range(1, n_pt_bins_truth-3):
      ...
      self.plot_theta(jetR, sd_label, zcut, beta, min_pt_truth, max_pt_truth, plot_pythia=False)
      self.plot_theta(jetR, sd_label, zcut, beta, min_pt_truth, max_pt_truth, plot_pythia=True)

  #----------------------------------------------------------------------
  def plot_theta(self, jetR, sd_label, zcut, beta, min_pt_truth, max_pt_truth, plot_pythia=False):
    ...
    name = 'hMain_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth)
    h = getattr(self, name)
    ...
    h_shape = getattr(self, 'hResult_shape_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth))
    ...
    h_corr = getattr(self, 'hResult_trkeff_R{}_{}_{}-{}'.format(jetR, sd_label, min_pt_truth, max_pt_truth))
