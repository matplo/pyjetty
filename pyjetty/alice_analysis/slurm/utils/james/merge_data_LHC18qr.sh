...
do
   FILE_DIR=$OUTPUT_DIR/LHC18qr/570/LHC18q/$RUN
...
do
   FILE_DIR=$OUTPUT_DIR/LHC18qr/570/LHC18r/$RUN
   FILES=$( find "$FILE_DIR" -name "*.root" )
   hadd -f -j 20 $FILE_DIR/AnalysisResultsIntermediate.root $FILES
done

FILES=$( find $OUTPUT_DIR/LHC18qr/570 -name "AnalysisResultsIntermediate.root" )
