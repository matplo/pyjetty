...
def main():
  method = "bayes"
  f1='$HOME/devel/pyjetty/pyjetty/cstoy/output_alpha_0_dRmax_0.4_SDzcut_0.2_emb.root'
  f2='$HOME/devel/pyjetty/pyjetty/cstoy/output_alpha_0_dRmax_0.2_SDzcut_0.2_emb.root'
  if len(sys.argv) > 1: method = sys.argv[1]
  if len(sys.argv) > 2: f1 = sys.argv[2]
  if len(sys.argv) > 3: f2 = sys.argv[3]
  run(f1, f2, method)
