...
import ROOT
load_extra = ROOT.gSystem.Load("$PYJETTYDIR/cpptools/lib/libpyjetty_rutil.dylib")
...
class GridFastJetROOT(JetAnalysis):
	def __init__(self, **kwargs):
		...
		self.gridR = ROOT.RUtil.ParticleGrid(	self._nbinsX, -self.particle_eta_max, self.particle_eta_max,
												self._nbinsY, 0., 2. * ROOT.TMath.Pi())
		# gridR.fillParticles(parts_pythia)
		# _tmp = [gridR.fillParticle(p) for p in parts_pythia]
		super(GridFastJetROOT, self).__init__(**kwargs)

	def analyze_event(self, parts):
		self.gridR.Reset()
		_tmp = [self.gridR.fillParticle(p.eta(), p.phi(), p.e()) for p in parts]
		self.particles = [fj.PseudoJet(p.px(), p.py(), p.pz(), p.e()) for p in self.gridR.getGridParticles()]
...
def main(agrs):
	...
	print(grfj)
	grfjR = GridFastJetROOT(grid_size=0.01, jet_R=args.jet_R, jet_algorithm=fj.antikt_algorithm, particle_eta_max=args.max_eta)
	print(grfjR)
...
if __name__ == '__main__':
	...
	parser.add_argument('--gridR', default=False, action='store_true')
